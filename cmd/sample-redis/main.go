...
func main() {
	// create client
	cli := cache.NewClient("localhost", "paswd", 1)
	// create bucket
	cli.WithBucket(cache.NewBucket[Foo](cli, "foo1", cachecoding.NewJsonCoder())) // return fooBkt1
	cli.WithBucket(cache.NewBucket[Bar](cli, "bar1", cachecoding.NewJsonCoder())) // return barBkt1
	cli.WithBucket(cache.NewBucket[Foo](cli, "foo2", cachecoding.NewJsonCoder())) // return fooBkt2
	cli.WithBucket(cache.NewBucket[Bar](cli, "bar2", cachecoding.NewJsonCoder())) // return barBkt2
